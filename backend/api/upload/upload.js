...
class upload {
    ...
    async image(ctx){
        let res;
        const file=ctx.req.file;    //获取上传文件        ctx.request是koa2中context经过封装的请求对象，它用起来更直观和简单。
    ...
    async uploadFile(ctx){
        ...
        //const files_description=`${ctx.req.file[0].originalname}等${ctx.req.files.length}张图片`;
        const files_description=`我的文件描述`;
        let files=[];
