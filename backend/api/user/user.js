...
class user {
    ...
    async edit(ctx){
        ...
        if(is_admin&&is_exist&&is_exist.length==1){
            await mysqlJs.queryFromMysql(`UPDATE users SET email = '${user.email}',username = '${user.username}',status = '${user.status}',is_deleted = '${user.is_deleted}',avatar = '${user.avatar}' WHERE uid = '${uid}'`);
            let userInfo=await mysqlJs.queryFromMysql(`SELECT * FROM users WHERE uid = '${uid}'`);
