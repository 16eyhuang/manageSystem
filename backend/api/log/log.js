...
class log {
    async insertOperationLog(ctx){
        ...
        let currentPage=record.request.data?record.request.data.currntPage||'':'';
        let pageSize=record.request.data?record.request.data.pageSize||'':'';
        let operationDescription = new Map([
            ['/api/user/login',[status===200?`${record.request.data.username}登录了平台`:`未知用户账号${record.request.data.username},密码${record.request.data.password}登录平台未遂`]],
