<template>
  <div class="vUpload">
    ...
    <div class="vUpload-button">
      ...
      <input type="file" style="display: none" ref="uploadBtn" @change="confirmUpload" multiple="multiple" accept="image/*">
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods:{
      uploadFile(){
        this.$alert('上传文件功能仅供学习交流之用，所上传的文件将在三个月之后删除。(目前只能上传图片，请不要上传奇♂怪的东西哦→_→)',
        '提示',
          {
        ...
        if(this.$refs.uploadBtn.files.length&&this.$refs.uploadBtn.files.length<100){
          ...
          this.$axios({
            ...
            if(res.data.status===200){
              this.$tips({
                type:'success',
                message:`已成功上传${this.$refs.uploadBtn.files.length}张图片`
              });
