...
<template>
  <div class="navBar">
    <el-menu class="el-menu-vertical-demo" :default-active="defaultActive" :collapse="isCollapse"
             background-color="#545c64" text-color="#fff" active-text-color="#409EFF" router unique-opened>
      <template v-for="(item,index) in $router.options.routes[1].children">
        <el-submenu :index="item.path" v-if="item.children&&item.children.length>0" :key="index">
          ...
          <el-menu-item v-for="(child,idx) in item.children" :index="child.path" :key='idx'>{{child.name}}
          </el-menu-item>
        </el-submenu>
