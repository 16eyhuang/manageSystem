        ...
        <el-row>
          <el-col :span="24">
            <div class="grid-content bg-purple-dark">
              <p>『 {{ hitokoto }} 』 —— 《{{ from }}》 </p>
              <el-button type="text" icon="el-icon-refresh" :loading="hitokotoLoading" :disabled="hitokotoLoading" @click="getHitokoto"></el-button>
<!--              <i class="el-icon-refresh" disabled="hitokotoLoading" @click='getHitokoto'></i>-->
            </div>
          </el-col>
        </el-row>
        <el-row :gutter="20">
    ...
    <div class="dynamic">
      <div class="github-time-line" v-loading='githubLoading'>
        ...
        <div class="github-time-line-item" v-for='(item, index) in currentGithubCommit' :key='index'>
          <div class="github-time-line-item-left">
            <img :src="item.author.avatar_url" alt="" height="32">
          </div>
          <div class="github-time-line-item-right">
            <p><span style="color:rgb(64, 158, 255)">{{ item.commit.committer.name }}</span> 在 {{currentGithubProject}}
              提交了内容为<span style="color:rgb(64, 158, 255)">{{ item.commit.message }}</span>的更新</p>
            <span>{{ item.commit.committer.date | formatterGithubCommitTime }}</span>
          </div>
        </div>
        <el-pagination layout="total, prev, pager, next" :total="githubCommitStatus.length" :page-size="5" @current-change="handleGithubCommitChange"></el-pagination>
      </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        currentGithubProject:`vue.js/vue`
      };
    },
    methods: {
      getHitokoto() {
      ...
      getGithubCommitStatus() {
        ...
        this.$axios
          ...
          .then(res => {
            ...
            this.currentGithubCommit=this.githubCommitStatus.slice(0,5);
      ...
      handleGithubCommitChange(val){
        this.currentGithubCommit=this.githubCommitStatus.slice((val-1)*5,(val-1)*5+5);
      }
    },
    filters: {
